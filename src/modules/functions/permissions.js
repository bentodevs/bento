...
exports.checkPerms = async (bot, message, permissions, cmd) => {
    ...
    permission = checkCat ? message.permissions.categories[cmd.info.category.toLowerCase()] : message.permissions.commands[cmd.info.name];
    ...
    if (message.guild) {
        if (permission.type == "role" && permission.hierarchic) {
            ...
            const position = await message.guild.roles.fetch(permission.permission).then(r => { return r.position; }).catch(() => {});
            ...
            if (position > message.member.roles.highest.position)
                return true;
