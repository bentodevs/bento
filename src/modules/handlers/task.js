 ...
 */
exports.reload = (bot, task) => {
    return new Promise((resolve, reject) => {
        // Get all the task files and get the specified task
        const files = readdirSync("./modules/tasks").filter(file => file.endsWith(".js")),
        file = files.find(e => e.toLowerCase() == `${task.toLowerCase()}.js`);
...
exports.load = (bot, task) => {
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            if (bot.tasks.get(taskName))
                return reject(new Error("Task is already loaded"));
 ...
 exports.unload = (bot, task) => {
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            // If the task is already loaded return an error
            if (!bot.tasks.get(taskName))
                return reject(new Error("Task isn't loaded"));
