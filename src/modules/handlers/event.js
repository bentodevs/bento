 ...
 */
exports.reload = (bot, event) => {
    return new Promise((resolve, reject) => {
        // Get all the event files and find the event specified
        const files = readdirSync("./events").filter(file => file.endsWith(".js")),
        file = files.find(e => e.toLowerCase() == `${event.toLowerCase()}.js`);
        ...
        if (file == "ready.js")
            return reject(new Error("The ready event can't be reloaded"));
...
exports.load = (bot, event) => {
    return new Promise((resolve, reject) => {
        ...
        // If the user specified the ready event return an error
        if (file == "ready.js")
            return reject(new Error("The ready event can't be loaded"));
        ...
        try {
            ...
            if (bot.listeners(eventName).length)
                return reject(new Error("Event is already loaded"));
 ...
 */
 exports.unload = (bot, event) => {
    return new Promise((resolve, reject) => {
        ...
        // If the user specified the ready event return an error
        if (file == "ready.js")
            return reject(new Error("The ready event can't be unloaded"));
        ...
        try {
            ...
            if (!bot.listeners(eventName).length)
                return reject(new Error("Event isn't loaded"));
