export default {
    ...
    run: async (bot, message, args) => {
        ...
        if ((args?.join(' ').length ?? message.options.get('text').value.length) >= 2000) return message.error('Your message was too long! Bots can only send messages with up to 2000 characters.');
