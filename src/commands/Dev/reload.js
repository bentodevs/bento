...
export default {
    ...
    run: async (bot, message, args) => {
        ...
        if (!args[1] || option === 'cmd') {
            ...
            if (!cmd) return message.errorReply("You didn't specify a valid command to reload!");
            ...
            const file = files.find((f) => f.toLowerCase() === `${args[1].toLowerCase()}.js`);

            // If the file doesn't exist return an error
            if (!file) return message.errorReply("You didn't specify a valid functions file!");
