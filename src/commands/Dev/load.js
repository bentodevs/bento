...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        if (option === 'event') {
            ...
            const category = option === 'cmd' ? args[2]?.toLowerCase() : args[1].toLowerCase();

            // If no command was specified return an error
            if (!cmd) return message.errorReply("You didn't specify a command!");
            // If no category was specified return an error
            if (!category) return message.errorReply("You didn't specify a category!");
