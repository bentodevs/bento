export default {
    ...
    run: async (bot, message, args) => {
        ...
        if (options.every((val, i, arr) => val.trim() === arr[0].trim())) return message.errorReply("You didn't really specify any options there?");
    ...
    run_interaction: async (bot, interaction) => {
        ...
        // If no options were specified or all options are the same return an error
        if (options.every((val, i, arr) => val.trim() === arr[0].trim())) return interaction.error("You didn't really specify any options there?");
