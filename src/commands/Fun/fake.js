...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        if (bot.config.general.devs.includes(member.id)) return message.errorReply('You cannot use this command on that member!');
        // If the member is a bot return an error
        if (member.user.bot) return message.errorReply('You cannot use this command on bots!');
        // If the members display name is shorter than 2 characters return an error
        if (member.displayName.length < 2) return message.errorReply('The name of the member you specified is shorter than 2 characters!');
        // If no message was specified return an error
        if (!args[1]) return message.errorReply("You didn't specify a message to send!");
    ...
    run_interaction: async (bot, interaction) => {
        ...
        // If no member was found return an error
        if (!member) return interaction.error({ content: "You didn't specify a valid member!", ephemeral: true });
        // If the member is a bot dev return an error
        if (bot.config.general.devs.includes(member.id)) return interaction.error({ content: 'You cannot use this command on that member!', ephemeral: true });
        // If the member is a bot return an error
        if (member.user.bot) return interaction.error({ content: 'You cannot use this command on bots!', ephemeral: true });
        // If the members display name is shorter than 2 characters return an error
        if (member.displayName.length < 2) return interaction.error({ content: 'The name of the member you specified is shorter than 2 characters!', ephemeral: true });
