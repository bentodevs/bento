...
export default {
    ...
    run: async (bot, message, args) => {
        ...
        const result = await urban(args.join(' '));

        // Send a error message if no definition was found
        if (!result) return msg.edit(`${bot.config.emojis.error} I couldn't find any definitions for that!`);

        // Build the embed
        const embed = new MessageEmbed()
            .setAuthor({ name: `Urban Dictionary: ${result.word}`, iconURL: 'https://i.imgur.com/kwkO5eD.jpg', url: result.permalink })
    ...
    run_interaction: async (bot, interaction) => {
        ...
        // Send a error message if no definition was found
        if (!result) return interaction.editReply(`${bot.config.emojis.error} I couldn't find any definitions for that!`);
