...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        if (args[0].toLowerCase() === 'reminders') {
            if (!await reminders.findOne({ _id: message.author.id })) return message.errorReply('You do not have any reminders for me to clear!');
            ...
            if (!await users.findOne({ _id: message.author.id })?.usernames?.length) return message.errorReply('You do not have any previous usernames for me to clear!');
            ...
            if (!await reminders.findOne({ _id: message.author.id }) && !await users.findOne({ _id: message.author.id })?.usernames?.length) return message.errorReply('You do not have any data for me to clear!');
