...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        if (role.position >= message.member.roles.highest.position) return message.error('You are unable to clone roles that are higher than, or equal to, your highest role!');

        // If the bot's highest role is below the role specified, then throw an error
        if (role.position >= message.guild.me.roles.highest.position) return message.error('I am unable to clone roles that are higher than, or equal to, my highest role!');
    ...
    run_interaction: async (bot, interaction) => {
        ...
        // If no role exists, then throw an error
        if (!role) return interaction.error({ content: "You didn't specify a valid role!", ephemeral: true });

        // If the user's highest role is below the role specified, then throw an error
        if (role.position >= interaction.member.roles.highest.position) return interaction.error('You are unable to clone roles that are higher than, or equal to, your highest role!');

        // If the bot's highest role is below the role specified, then throw an error
        if (role.position >= interaction.guild.me.roles.highest.position) return interaction.error('I am unable to clone roles that are higher than, or equal to, my highest role!');
