...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        if (!args[0]) {
            ...
            if (!message.settings.moderation.minimumAge)
                return message.errorReply("This guild does not have a minimum account age!");
            ...
            if (args[0].toLowerCase() == "disable") {
                ...
                if (!message.settings.moderation.minimumAge)
                    return message.errorReply("The minimum account age is already disabled!");
                ...
                if (message.settings.moderation.minimumAge == time)
                    return message.errorReply("The time you specified is already set as the minimum required account age!");
    ...
    run_interaction: async (bot, interaction) => {
        ...
        if (!option) {
            // If no minimum age is set return an error
            if (!interaction.settings.moderation.minimumAge)
                return interaction.error("This guild does not have a minimum account age!");
            ...
            if (interaction.options.get("time").value == "disable") {
                // If the minimum age is already disabled return an error
                if (!interaction.settings.moderation.minimumAge)
                    return interaction.error("The minimum account age is already disabled!");
                ...
                // If the specified time is the same as the current minimum age return an error
                if (interaction.settings.moderation.minimumAge == time)
                    return interaction.error("The time you specified is already set as the minimum required account age!");
