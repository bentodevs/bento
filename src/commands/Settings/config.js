...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        if (!option) {
            ...
            // Get the DB query
            const toUpdate = { "general.permission_message": message.settings.general.permission_message ? false : true };
            ...
            // Get the DB query
            const toUpdate = { "general.permission_dms": message.settings.general.permission_dms ? false : true };
            ...
            // Get the DB query
            const toUpdate = { "general.disabled_message": message.settings.general.disabled_message ? false : true };
            ...
            if (args.slice(1).join(" ").toLowerCase() == "default") {
                ...
                if (message.settings.general.timezone == "UTC")
                    return message.errorReply("The timezone of this guild is already set to the default!");
            ...
            if (!timezone)
                return message.errorReply({ content: "You didn't specify a valid timezone! Click the button below to view a list of timezones.", components: [row] });
            // If the specified timezone is already set return an error
            if (timezone.tzCode == message.settings.general.timezone)
                return message.errorReply(`The timezone of this guild is already set to \`${timezone.label}\`!`);
    ...
    run_interaction: async (bot, interaction) => {
        ...
        if (view) {
            ...
            // If an invalid timezone was specified return an error
            if (!timezone)
                return interaction.error({ content: "You didn't specify a valid timezone! Click the button below to view a list of timezones.", components: [row] });
            // If the specified timezone is already set return an error
            if (timezone.tzCode == interaction.settings.general.timezone)
                return interaction.error(`The timezone of this guild is already set to \`${timezone.label}\`!`);
