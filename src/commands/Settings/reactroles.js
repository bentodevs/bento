...
export default {
    ...
    run: async (bot, message, args) => {
        ...
        if (!option) {
            ...
            if (!args[2]) return message.errorReply("You didn't specify a message id!");
            // If the user didn't specify a emoji return an error
            if (!args[3]) return message.errorReply("You didn't specify a emoji!");
            // If the user didn't specify a role return an error
            if (!args[4]) return message.errorReply("You didn't specify a role!");

            // Grab the message, emote and role
            const msg = await channel.messages.fetch(args[2]).catch(() => {});
            const emote = getEmoji(message.guild, args[3]);
            const role = await getRole(message, args.slice(4).join(' '));
            ...
            if (data) {
                ...
                if (data.roles.find((r) => r.role === role.id)) return message.errorReply('The role you specified is already added to this message!');
                // If the emoji is already in the array return an error
                if (data.roles.find((r) => (r.emoji === emote.id ? emote.id : emote.emoji))) return message.errorReply('The emoji you specified is already added to this message!');
            ...
            if (!reactMenu) return message.errorReply("You didn't specify a message ID which is being used for a react message!");
