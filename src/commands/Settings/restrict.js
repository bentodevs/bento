...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        if (!args[1]) {
            ...
            if (!data)
                return message.errorReply("Looks like there aren't any restrictions on that channel!");
            ...
            if (!types.includes(args[1].toLowerCase()))
                return message.errorReply(`You didn't specify a valid type! Try one of these: \`${types.join("`, `")}\``);
    ...
    run_interaction: async (bot, interaction) => {
        ...
        if (!interaction.options.get("type")?.value) {
            ...
            // If no data was found return an error
            if (!data)
                return interaction.error("Looks like there aren't any restrictions on that channel!");
