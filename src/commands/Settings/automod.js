...
export default {
    ...
    run: async (bot, message, args) => {
        if (!args[0]) {
            ...
            if (!message.settings.moderation?.no_invite) {
            ...
            if (!message.settings.moderation?.no_link) {
            ...
            if ((args[1]?.toLowerCase() === 'off') || (args[1].toLowerCase() === '0')) {
                ...
                if (!message.settings.moderation.mentions_mute) return message.errorReply('There is no mute threshold for message mentions currently set!');
                ...
                return message.errorReply('You did not specify a valid number, or `off`!');
            }
        } else if (args[0].toLowerCase() === 'mentions-ban') {
            if ((args[1]?.toLowerCase() === 'off') || (args[1]?.toLowerCase() === '0')) {
                // If the max mentions is unset already, return an error
                if (!message.settings.moderation.mentions_ban) return message.errorReply('There is no ban threshold for message mentions currently set!');
                ...
                if (message.settings.moderation.mentions_mute && (args[1] < parseInt(message.settings.moderation.mentions_mute, 10))) return message.errorReply('The mentions-ban amount needs to be higher than the mentions-mute setting!');
    ...
    run_interaction: async (bot, interaction) => {
        ...
        if (sub === 'view') {
            ...
            if (interaction.options.get('amount').value === 0) {
                // If the max mentions is unset already, return an error
                if (!interaction.settings.moderation.mentions_mute) return interaction.error('There is no mute threshold for message mentions currently set!');
                ...
                // If the amount specified is larger than the mentions_ban setting return an error
                if (interaction.settings.moderation.mentions_ban && interaction.options.get('amount').value > interaction.settings.moderation.mentions_ban) return interaction.error('The mentions-mute amount needs to be lower than the mentions-ban setting!');
            ...
            if (interaction.options.get('amount').value === 0) {
                // If the max mentions is unset already, return an error
                if (!interaction.settings.moderation.mentions_ban) return interaction.error('There is no ban threshold for message mentions currently set!');
                ...
                // If the amount specified is smaller than the mentions_mute setting return an error
                if (interaction.settings.moderation.mentions_mute && interaction.options.get('amount').value < interaction.settings.moderation.mentions_mute) return interaction.error('The mentions-ban amount needs to be higher than the mentions-mute setting!');
