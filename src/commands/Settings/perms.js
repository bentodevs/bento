...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        const category = categories[categories.indexOf(args[0].toLowerCase())];

        // If no command was found or the command is a dev command return an error
        if ((!command && !category) || (command?.info.category.toLowerCase() === 'dev' || category === 'dev')) return message.errorReply("You didn't specify a valid category or command!");
        ...
        const location = command ? `permissions.commands.${command.info.name}` : `permissions.categories.${category}`;
        const target = command ? message.settings.general.prefix + command.info.name : category;
        ...
        if (!args[1]) {
            ...
            if (args[1].toUpperCase() === perm?.permission) return message.errorReply(`The permission for the \`${target}\` ${type} is already set to \`${args[1].toUpperCase()}\`!`);
            ...
            if (!perm?.permission || JSON.stringify(perm) === JSON.stringify(command ? filterSelfPerms(command.perms) : null)) return message.errorReply(`The ${type} is already set to the default permission!`);
            ...
            if (!role) return message.errorReply("You didn't specify a valid role or permission!");
            ...
            if (args.slice(1).join(' ').toLowerCase().includes('+') && (role.name.match(/\+/g) || []).length < (args.slice(1).join(' ').match(/\+/g) || []).length) {
                ...
                if (role.id === perm?.permission && perm?.hierarchic) return message.errorReply(`The ${type} is already set to the permission you specified!`);
            ...
            } else if (Array.isArray(perm?.permission)) {
    ...
    run_interaction: async (bot, interaction) => {
        ...
        const command = bot.commands.get(opt) || bot.commands.get(bot.aliases.get(opt));
        const category = categories[categories.indexOf(opt)];

        // Some useful variables to make this command 3x shorter xd
        const type = command ? 'command' : 'category';
        const location = command ? `permissions.commands.${command.info.name}` : `permissions.categories.${category}`;
        const target = command ? interaction.settings.general.prefix + command.info.name : category;
        const perm = interaction.permissions.categories[category] || interaction.permissions.commands[command?.info.name] || undefined;

        // If no command was found or the command is a dev command return an error
        if ((!command && !category) || (command?.info.category.toLowerCase() === 'dev' || category === 'dev')) return interaction.error(`You didn't specify a valid ${command ? 'command' : 'category'} !`);
        ...
        if (data === 'view') {
            ...
            const permission = checkCat ? interaction.permissions.categories[command.info.category.toLowerCase()] : interaction.permissions.commands[command.info.name];
            ...
            // If no role was specified return an error
            if (!role) return interaction.error("You didn't specify a valid role!");

            if (interaction.options.get('hierarchic')?.value) {
                // If the permission is already set to the role specified return an error
                if (role.id === perm?.permission && perm?.hierarchic) return interaction.error(`The ${type} is already set to the permission you specified!`);
            ...
            if (!Permissions.FLAGS[permission]) return interaction.error({ content: "You didn't specify a valid Discord Permission! Click the button below to view a list of permissions.", components: [row] });
            // If the permission is already set to the specified permission return an error
            if (permission === perm?.permission) return interaction.errir(`The permission for the \`${target}\` ${type} is already set to \`${permission}\`!`);
            ...
            // If the permission is already set to default return an error
            if (!perm?.permission || JSON.stringify(perm) === JSON.stringify(command ? filterSelfPerms(command.perms) : null)) return interaction.error(`The ${type} is already set to the default permission!`);
