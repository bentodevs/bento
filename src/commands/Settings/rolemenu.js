    ...
    run: async (bot, message, args) => {
        ...
        if (!args[0]) {
            ...
            if (!reactRoles.length)
                return message.error("There aren't any reaction menus setup for this guild!");
            ...
            if (!args[1])
                return message.error("You need to specify a message ID to remove!");
            ...
            if (!reactMenu)
                return message.error("You didn't specify a message ID which is being used for a react menu!");
            ...
            opts = args.slice(1).join(" ").split("||"),
            description = args.join(" ").includes("||") ? opts[0] : null,
            options = opts[1]?.split("|") ?? args.slice(1).join(" ").split("|"),
            array = [];
            ...
            if (!channel)
                return message.error("You didn't specify a valid channel! *(You must mention a channel!)*");
            // If no options were specified return an error
            if (!options)
                return message.error("You didn't specify any roles/emojis!");
            ...
            for (const data of options) {
                ...
                emote = getEmoji(message.guild, i[0]),
                role = await getRole(message, i.slice(1).join(" "));

                // If no emote was found return an error
                if (!emote)
                    return message.error("You didn't specify a valid emoji! *You must specify a emoji from this Discord or a default Discord emoji.*");
                ...
                if (message.guild.me.roles.highest.position <= role.position) 
                    return message.error("One of the roles you specified is higher than or equal to my highest role!");
                // If the role is higher than the users highest role send an error
                if (message.member.roles.highest.position <= role.position)
                    return message.error("One of the roles you specified is higher than or equal to your highest role!");
