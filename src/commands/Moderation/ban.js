...
export default {
    ...
    run: async (bot, message, args) => {
        ...
        if (member.id === message.author.id) return message.errorReply('You are unable to ban yourself!');
        ...
        if (member.guild) {
            ...
            if (!member.bannable) return message.errorReply('I am not able to ban this member! *They may have a higher role than me!*');
            ...
            if (await preban.findOne({ user: member.id })) return message.errorReply('That user will already be banned if they join the server!');
    ...
    run_interaction: async (bot, interaction) => {
        ...
        if (user.member) {
            ...
            if (!interaction.guild.members.cache.get(user.user.id).bannable) return interaction.reply("I can't ban that member! *They may have a higher role than me!*");
            ...
            // Send public ban log message, if it exists
            interaction.guild.channels.fetch(interaction.settings.logs?.ban).then(async (channel) => {
            ...
            // If the member is not part of the server, but the ID does exist, then we'll add them to the pre-ban database
            // This means that if/when they join the guild, they will be banned
            if (await preban.findOne({ user: user.user.id })) return interaction.error('That user will already be banned if they join the server!');
