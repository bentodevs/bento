...
export default {
    ...
    run: async (bot, message, args) => {
        ...
        if (!time && time !== 0) return message.errorReply("You didn't specify a valid time!");
        // If the time is higher than 6 hours return an error
        if (time > 21600) return message.errorReply('The slow mode cannot be higher than 6 hours!');
    ...
    run_interaction: async (bot, interaction) => {
        ...
        // If the time is higher than 6 hours return an error
        if (time > 21600) return interaction.error('The slow mode cannot be higher than 6 hours!');
