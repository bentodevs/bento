...
export default {
    ...
    run: async (bot, message, args) => {
        ...
        if (option === 'list') {
            ...
            if (!g.length) return message.errorReply('There are no giveaways to list!');
            ...
            let channel; let time; let winners; let
                prize;
            ...
            if (!channel) return message.errorReply("You didn't specify a valid channel! The giveaway has been canceled.");
            // If the channel isn't a news or text channel return an error
            if (channel.type !== 'GUILD_TEXT' && channel.type !== 'GUILD_NEWS') return message.errorReply("The channel you specified isn't a text or news channel. The giveaway has been canceled.");
            ...
            if (!time) return message.errorReply("You didn't specify a valid time! The giveaway has been canceled.");
            // If the duration is shorter than 1 minute return an error
            if (time < 60000) return message.errorReply('The minumum time for a giveaway is 1 minute! The giveaway has been canceled.');
            // If the duration is longer than 1 year return an error
            if (time > 31556952000) return message.errorReply('Giveaways cannot run for longer than 1 year! The giveaway has been canceled.');
            ...
            await message.channel.awaitMessages(options).then(async (msgs) => {
                ...
                winners = parseInt(m.content, 10);
            }).catch(() => {
                message.errorReply('The giveaway has been canceled.');
            });

            // If no winners weres specified return an error
            if (!winners) return message.errorReply("You didn't specify a valid number! The giveaway has been canceled.");
            // If more than 20 winners were specified return an error
            if (winners > 20) return message.errorReply("You can't have more than 20 winners! The giveaway has been canceled.");
            // If 0 or less winners were specified return an error
            if (winners <= 0) return message.errorReply('You must have at least 1 winner! The giveaway has been canceled.');
            ...
            if (!prize) return message.errorReply("You didn't specify a prize! The giveaway has been canceled.");
            ...
            if (`Giveaway: ${prize}`.length > 256) return message.errorReply('Please enter a shorter prize name! The giveaway has been canceled.');
            ...
            if (!id) return message.errorReply('You must provide a numeric giveaway ID!');

            // Get the giveaway
            const g = await giveaways.findOne({ 'guild.guild_id': message.guild.id, id, active: true });
            ...
            await giveaways.findOneAndUpdate({ 'guild.guild_id': message.guild.id, id, active: true }, {
                active: false,
            });
            ...
            // Get the giveaway
            const g = await giveaways.findOne({ 'guild.guild_id': message.guild.id, id, active: false });

            // If the giveaway wasn't found return an error
            if (!g) return message.errorReply('There is no giveaway with that ID or the giveaway is still active!');
    ...
    run_interaction: async (bot, interaction) => {
        ...
        if (sub === 'list') {
            ...
            // If there are no giveaways return an error
            if (!g.length) return interaction.error('There are no giveaways to list!');
            ...
            const time = parseTime(interaction.options.get('duration').value, 'ms');
            ...
            if (time > 31556952000) return interaction.error('The maximum duration for a giveaway is 1 year!');

            // Winner Checks
            if (winners > 20) return interaction.error('The maximum winners for a giveaway is 20!');
            if (winners <= 0) return interaction.error('The giveaway should at least have 1 winner!');

            // Prize Checks
            if (`Giveaway: ${prize}`.length > 256) return interaction.error('Please enter a shorter prize name!');
            ...
            // Get the giveaway
            const g = await giveaways.findOne({ 'guild.guild_id': interaction.guild.id, id, active: true });
            ...
            // Set the giveaway to inactive in the db
            await giveaways.findOneAndUpdate({ 'guild.guild_id': interaction.guild.id, id, active: true }, {
                active: false,
            });
            ...
            // Get the giveaway
            const g = await giveaways.findOne({ 'guild.guild_id': interaction.guild.id, id, active: false });

            // If the giveaway wasn't found return an error
            if (!g) return interaction.error('There is no giveaway with that ID or the giveaway is still active!');
