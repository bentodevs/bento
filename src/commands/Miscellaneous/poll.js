...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        if (channel.type !== 'GUILD_TEXT' && channel.type !== 'GUILD_NEWS') return message.errorReply("The channel you mentioned isn't a text or news channel!");
        // If the user doesn't have MANAGE_MESSAGES permission but specified a different channel return an error
        if (channel.id !== message.channel.id && !message.channel.permissionsFor(message.member).has('MANAGE_CHANNELS')) return message.errorReply('To send polls to different channels you need the `MANAGE_CHANNELS` permission!');
        // If the user can't send messages in the specified channel return an error
        if (!channel.permissionsFor(message.member).has('SEND_MESSAGES')) return message.errorReply("You don't have permissions to send messages in that channel!");

        // Poll Vars
        const poll = args.join(' ').split('|');
        const question = poll[0];
        const choices = poll.splice(1);

        // If no choices were specified return an error
        if (choices.length <= 0) return message.errorReply("You didn't specify any options!");
        // If more than 10 choices were specified return an error
        if (choices.length > 10) return message.errorReply('YOu cannot specify more than 10 options!');
        // If the choices are the same return an error
        if (choices.every((val, i, arr) => val.trim() === arr[0].trim())) return message.errorReply('You should at least give them a choice 🤔');
        ...
        if (channel.id !== message.channel.id) message.confirmationReply(`The poll has successfully been set to ${channel}!`);
    ...
    run_interaction: async (bot, interaction) => {
        ...
        // If the user doesn't have MANAGE_MESSAGES permission but specified a different channel return an error
        if (channel.id !== interaction.channel.id && !interaction.channel.permissionsFor(interaction.member).has('MANAGE_CHANNELS')) return interaction.error('To send polls to different channels you need the `MANAGE_CHANNELS` permission!');
        // If the user can't send messages in the specified channel return an error
        if (!channel.permissionsFor(interaction.member).has('SEND_MESSAGES')) return interaction.error("You don't have permissions to send messages in that channel!");

        const question = interaction.options.get('question').value;
        const choices = interaction.options.filter((a) => a.name.startsWith('option')).map((a) => a.value);

        // If the choices are the same return an error
        if (choices.every((val, i, arr) => val.trim() === arr[0].trim())) return interaction.error('You should at least give them a choice 🤔');
