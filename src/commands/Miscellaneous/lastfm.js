...
module.exports = {
    ...
    run: async (bot, message, args) => {
        if (!args[0]) {
            ...
            if (!userData.lastfm) return message.errorReply(`You do not have a Last.fm account set! Set one with \`${message.settings.general.prefix}lastfm set <username>\``);
            ...
            if (!latestTrack?.['@attr'].nowplaying) return message.errorReply("It looks like you aren't playing anything right now!");
            ...
            if (regex.test(args[1])) return message.errorReply('Your Last.fm username cannot have any special characters!');
            ...
            if (!member) return message.error("I couldn't find the specified user!");

            // If the user doesn't have a LastFM profile set, then return an error
            if (!user?.lastfm) return message.error('That user does not have a Last.fm account linked!');
    ...
    run_interaction: async (bot, interaction) => {
        ...
        if (sub === 'playing') {
            ...
            if (!userData.lastfm) return interaction.editReply({ content: `${config.emojis.error} You do not have a Last.fm account set!` });
            ...
            if (!latestTrack['@attr']?.nowplaying) return interaction.editReply({ content: `${config.emojis.error} It looks like you aren't playing anything right now!` });
            ...
            if (!newName) {
                ...
                if (!userData.lastfm) return interaction.error({ content: 'You do not have a Last.fm account set!', ephemeral: true });
            ...
            if (regex.test(newName.value)) return interaction.editReply({ content: `${config.emojis.error} Your Last.fm username cannot have any special characters!` });
            ...
            if (user) {
                ...
                if (!userData?.lastfm) return interaction.editReply({ content: `${config.emojis.error} That user doesn't have a Last.fm account set!` });
